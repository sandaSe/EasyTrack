...
def login_page():
    ...
    if st.button("Login"):
        if check_login(username, password):
            ...
            st.session_state["current_page"] = "Settings"  # Set the default page after login
...
if not st.session_state["authenticated"]:
    ...
    menu = option_menu(
        ...
        menu_icon="cast",
        default_index=["Settings", "Dashboard", "Expense Hub", "Insights"].index(st.session_state["current_page"]),
        orientation="horizontal",
        styles={
            "container": {"padding": "5px", "background-color": "#f0f2f6"},  # Adjust container padding and background
            "icon": {"color": "#3e8a56", "font-size": "20px"},
            "nav-link": {"font-size": "16px", "text-align": "left", "margin": "10px", "color": "#000"},
            "nav-link-selected": {"background-color": "#f00", "color": "#fff"},  # Highlight selected menu item
        }
    )
    ...
    if menu == "Settings":
        ...
        home.add_sidebar("Images/2.png")
        home.main()
    elif menu == "Expense Hub":
        exp_app.add_sidebar("Images/3.png")
        exp_app.main()
    elif menu == "Insights":
        analytics.add_sidebar("Images/4.png")
        analytics.main()
