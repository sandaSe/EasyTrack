...
def main():
    ...
    predefined_categories = ["Food", "Transportation", "Housing", "Shopping", "Healthcare", "Entertainment", "Other"]

    df, category_totals, _ = load_data()

    for category in predefined_categories:
        if category not in df["category"].unique():
            df = df.append({"category": category, "cumulative_sum": 0, "date": None}, ignore_index=True)
    ...
    if not df.empty:
        bar_data = pd.DataFrame(list(category_totals.items()), columns=["category", "category_totals"])
    ...
    if not df.empty:
        fig = px.line(
            ...
            labels={"cumulative_sum": "Amount Spent (LKR)", "date": "Date", "category": "Category"},
            markers=True
        )
        fig.update_layout(template="plotly_white")
        st.plotly_chart(fig)
