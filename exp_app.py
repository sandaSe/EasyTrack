...
def main():
   ...
   # st.header("Expense Hub")
    ...
    with tab1:
        ...
        selected_date = st.date_input("Date", value=datetime.now().date(), key='date_physical')
        ...
        if uploaded_file:
            ...
            if "image" in file_type:  # If the uploaded file is an image
                ...
                st.write(f"{file_type.split('/')[-1].upper()} file uploaded successfully.")
        ...
        if st.button("Submit", key="submit_physical"):
            if uploaded_file:
                ...
                input_variables = process_image_with_document_ai(
                    ...
                    file_path=file_path,
                    key_path="expense-manager-443707-f48c405eeb96.json"
                )
        ...
        if st.session_state.view_pressed:
            ...
            bill_date = st.date_input("Date", value=pd.to_datetime(df['Date'].iloc[0]).date(), key = "bill_date")
            ...
            st.number_input("Bill Total", value=df['Total'].iloc[0])


            # Initialize columns for both the original and converted prices
            df['Original Price'] = df['Price']  # Keep the original price
            total_original = df['Total'].iloc[0]
            total_in_lkr = None

            if currency != "LKR":
              df['Price in LKR'] = df['Price'].apply(lambda x: convert_currency(currency, "LKR", x))
              total_in_lkr = convert_currency(currency, "LKR", total_original)
            col1, col2 = st.columns(2)
            col1.metric(label=f"Total Bill ({currency})", value=f"{total_original:.2f} {currency}")
            ...
            if 'Price in LKR' in df.columns:
                df = df[['Category', 'Item', 'Original Price', 'Price in LKR']]  # Show only relevant columns
            else:
                df = df[['Category', 'Item', 'Original Price']]  # If no conversion, show just the original price
            ...
            if st.button("Confirm"):
                ...
                cycle_id = add_new_cycle(start_date=selected_date)  # Add a new spending cycle
    ...
    # Tab 2: E-Bills
    with tab2:
        selected_date = st.date_input("Date", value=datetime.now().date(), key='date_ebill')

        st.subheader("Digital Receipts")
        e_bill_text = st.text_area("Copy and paste your digital receipt details below:", height=150)
    ...
    # Tab 3: Manual Entry
    with tab3:
        selected_date = st.date_input("Date", value=datetime.now().date(), key='date_manual')

        st.subheader("Manual Entries")
        categories = ["Food", "Transportation", "Housing", "Shopping", "Healthcare","Entertainment", "Others"]
        selected_category = st.selectbox("Select Expense Category", categories)

        st.subheader("Add Items (Optional)")

        if "expenses_df" not in st.session_state:
            st.session_state.expenses_df = pd.DataFrame(columns=["Item Name", "Item Price"])

        with st.form(key="expense_form"):
            for i in range(len(st.session_state.expenses_df)):
                row = st.session_state.expenses_df.iloc[i]
                item_name = st.text_input(f"Item {i+1} Name", value=row["Item Name"], key=f"item_name_{i}")
                item_price = st.number_input(f"Item {i+1} Price", value=row["Item Price"], key=f"item_price_{i}", min_value=0.0, step=0.01)
        ...
        if st.button("Submit Manual Entry", key=33):
            if total_amount > 0:
                st.success(f"Manual entry submitted successfully for category '{selected_category}'!")
            else:
